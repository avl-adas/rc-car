...
const float DIST_TO_OBSTACLE_LO = 30.0F;
const float CRUISE_VELOCITY = 60.0F; // in cm/sec
const float HARD_BRAKE = -30.0F; 	 // in cm/sec

void Speed_Control(float reference_speed);
void ACC_Func_Handler();
void ACC(float ultrasonic_data, int sample_time, bool brake_flag);

//Undecided
void Sensor_read();
void Mode_Selector();
float speed_command_arbitration(float cruise_control, float pi_speed_cmd, float remote_speed);
#endif
