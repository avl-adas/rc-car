...
void loop() {
  ...
  PWM->PWM_CH_NUM[2].PWM_CDTYUPD = i;      //Set D36 PPM%
  PWM->PWM_CH_NUM[3].PWM_CDTYUPD = 3000-i; //Set D38 PPM%
...
void PWM_SERVO_SETUP()
{
  ...
  PWM->PWM_CH_NUM[3].PWM_CMR = PWM_CMR_CALG | PWM_CMR_CPRE_CLKA;      // Enable dual slope PWM and set the clock source as CLKA
  PWM->PWM_CH_NUM[3].PWM_CPRD = 20000;                                // Set the PWM frequency 2MHz/(2 * 20000) = 50Hz;
  
  REG_PWM_ENA = PWM_ENA_CHID2 | PWM_ENA_CHID3;                        //Enable PWM channels 2 and 3;
  
  delay(1);
  PWM->PWM_CH_NUM[2].PWM_CDTYUPD = 1580;        // Set initial PWM
  PWM->PWM_CH_NUM[3].PWM_CDTYUPD = 1580;  
  ...
  PWM->PWM_CH_NUM[3].PWM_CDTYUPD = 1500;  
  delay(3000);
}
